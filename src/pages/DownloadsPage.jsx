...
function buildTitleSelect(dl, onSelect, forDownload = false) {
  if (!dl.tmdbId || !dl.mediaType || !onSelect) return null;
  return () =>
    onSelect(
      {
        id: dl.tmdbId,
        ...
        season:
          dl.mediaType === "tv" && dl.season != null
            ...
            : undefined,
        episode:
          dl.mediaType === "tv" && dl.episode != null
            ? Number(dl.episode)
            : undefined,
      },
      forDownload ? { forDownload: true } : undefined,
    );
}
          ...
          <div className="dl-page__list">
            {active.map((dl) => (
              <ActiveCard
                ...
                onSelect={buildTitleSelect(dl, onSelect, true)}
              />
            ))}
          </div>
        </div>
            ...
            {allLocalItems.map((dl) => {
              ...
              return (
                <LocalFileCard
                  ...
                  onSelect={
                    dl.status === "error"
                      ? buildTitleSelect(dl, onSelect, true)
                      : buildTitleSelect(dl, onSelect, false)
                  }
      ...
      {downloads.length === 0 &&
        ...
        active.length === 0 && (
          <div className="empty-state">
            ...
            <p>
              Open a movie or episode, press Download, let the video play briefly,
              then start the download from the panel below the player.
            </p>
          </div>
        )}
    </div>
