...
export default function MoviePage({
  ...
  const enrichedRatingsMap = useMemo(
    () => ({
      [`movie_${item.id}`]: {
        ...
        restricted: isRestricted(rating.minAge, ageLimitSetting),
      },
    }),
    [item.id, rating.cert, rating.minAge, ageLimitSetting],
  );
  ...
  const startDownloadFlow = useCallback(() => {
    ...
    setShowSourceMenu(false);
    if (!playing && !restricted && !isUnreleased) {
      handlePlay();
    }
    setShowDownload(true);
    requestAnimationFrame(() => {
      document
        .getElementById("movie-download-panel")
  ...
  useEffect(() => {
    if (!downloadOnMount || restricted || isUnreleased) return;
    startDownloadFlow();
    onDownloadIntentHandled?.();
  }, [
    downloadOnMount,
  ...
  return (
    ...
    >
              ...
              {!isUnreleased && !restricted && (
                <button
                  ...
                  title={
                    movieDownload
                      ? "View in Downloads"
                      : "Open player and download this movie"
                  }
                >
                  <DownloadIcon />
                  {movieDownload
      ...
      {showDownload && playing && (
        <div id="movie-download-panel" className="movie-download-panel section">
          <DownloadModal
            variant="panel"
