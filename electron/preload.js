/** Preload bridge for the main Mov renderer (contextIsolation on). */
...
contextBridge.exposeInMainWorld("electron", {
  ...
  onM3u8Found: (cb) => {
    const h = (_, data) => {
      if (typeof data === "string") cb({ url: data, referer: "" });
      else cb(data || { url: "", referer: "" });
    };
  ...
  getSecureStoreInfo: () => ipcRenderer.invoke("get-secure-store-info"),
