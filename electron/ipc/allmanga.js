/**
 * AllManga episode resolver and localhost-only HLS proxy player.
 */
...
const { validateProxyTargetUrl } = require("../security/paths");
  ...
  return new Promise((resolve, reject) => {
    const server = http.createServer((req, res) => {
      ...
      if (url.pathname === "/proxy") {
        ...
        const targetCheck = validateProxyTargetUrl(target);
        if (!targetCheck.ok) {
          res.writeHead(400);
          res.end();
          return;
        ...
        try {
          const targetUrl = new URL(targetCheck.url);
